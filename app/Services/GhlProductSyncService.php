...
class GhlProductSyncService
{
    ...
    public function bulkPullFromGhl(string $tenantId): array
    {
        ...
        try {
            ...
            $results['deactivated'] = $this->deactivateMissingProducts($tenantId, $seenGhlIds);
        } catch (\Exception $e) {
            $results['errors']++;
     ...
     * now."
     */
    private function deactivateMissingProducts(string $tenantId, array $seenGhlIds): int
    {
        if (empty($seenGhlIds)) {
            return 0;
        }

        $stale = Product::byTenant($tenantId)
            ->whereNull('product_rental_id')
            ->whereNotNull('ghl_product_id')
    ...
    public function pullCategoriesFromGhl(string $tenantId): array
    {
        ...
        try {
            ...
            $results['deactivated'] = $this->deactivateMissingCategories($tenantId, $seenGhlIds);
        } catch (\Exception $e) {
            $results['errors']++;
     ...
     */
    private function deactivateMissingCategories(string $tenantId, array $seenGhlIds): int
    {
        if (empty($seenGhlIds)) {
            return 0;
        }
    ...
    private function createLocalStubIfMissing(array $ghlProduct, string $tenantId): bool
    {
        ...
        // (Product::byTenant()->...->get(), which only sees non-trashed
        // rows) pick this product back up and refresh its data moments
        // later in the same pull run.
        $trashedMatch = Product::onlyTrashed()->byTenant($tenantId)->where('ghl_product_id', $ghlId)->first();
        if ($trashedMatch) {
        ...
        $exists = Product::byTenant($tenantId)
