...
class GhlFullSyncService
{
    ...
    private function upsertPaidInvoice(string $tenantId, array $invoice, array $bookingIndex): array
    {
        ...
        foreach ($invoice['invoiceItems'] ?? [] as $item) {
            ...
            $rental = $productId ? ProductRental::where('tenant_id', $tenantId)->where('ghl_product_id', $productId)->first() : null;

            $product = null;
            if (! $rental) {
                $product = $productId
                    ? Product::where('tenant_id', $tenantId)->where('ghl_product_id', $productId)->whereNull('product_rental_id')->first()
                    : null;
            }
            ...
            if (! $rental && ! $product && ! $productId && $name) {
                $rental = ProductRental::where('tenant_id', $tenantId)
                    ->whereHas('product', fn ($q) => $q->whereRaw('LOWER(name) = ?', [strtolower($name)]))
                    ->first();
        ...
        if (! empty($rentalLines)) {
            ...
            if (! $localBooking) {
                ...
                RentalTransaction::updateOrCreate(
                    ['tenant_id' => $tenantId, 'ghl_invoice_id' => $ghlInvoiceId],
                    array_merge([
    ...
    private function resolveLocalBooking(
        ...
        $existing = Booking::where('tenant_id', $tenantId)
    ...
    private function resolveLocalCustomer(string $tenantId, array $contact): ?Customer
    {
        $ghlContactId = $contact['id'] ?? null;
        $email = $contact['email'] ?? null;

        if ($ghlContactId) {
            $customer = Customer::where('tenant_id', $tenantId)->where('ghl_contact_id', $ghlContactId)->first();
            if ($customer) {
                return $customer;
            }
        }
        ...
        if ($email) {
            return Customer::where('tenant_id', $tenantId)->whereRaw('LOWER(email) = ?', [strtolower($email)])->first();
        }
    ...
    private function reconcileExistingRecords(string $tenantId): array
    {
        ...
        $pendingProductTransactions = ProductTransaction::where('tenant_id', $tenantId)
            ->whereNotNull('ghl_invoice_id')
            ->where(function ($q) {
    ...
    /** Used by the daily scheduled command to loop every configured tenant. */
    public static function tenantIdsWithEngageSettings(): array
    {
        return EngageSetting::whereNotNull('location_id')->pluck('tenant_id')->unique()->values()->all();
    }
}
